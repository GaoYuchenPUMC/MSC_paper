...
design=read.csv('/data/gaoyuchen/NC_Revision3_GSVA/Design_cluster_GSVA.csv',row.names = 'symbol')


####GSVA analysis####
geneset <- getGmt("/data/gaoyuchen/NC_Revision3_GSVA/GSVA_GeneSets.gmt")

gsva_mat <- gsva(expr=pseudobulk_MSC, 
                              gset.idx.list=geneset, 
                              kcdf="Poisson",  #"Gaussian" for logCPM,logRPKM,logTPM, "Poisson" for counts
                              verbose=T)

####Pseudobulk differential analysis using limma with batch correction####
library(dplyr)
pseudobulk_cluster <- c(rep("C1", 12), rep("C2", 12),rep("C3", 12),rep("C4", 12),
          rep("C5", 12),rep("C6", 12),rep("C7", 12)) %>% factor(., levels = c('C1','C2','C3','C4','C5','C6','C7'), ordered = F)

batch <- factor(rep(rep(1:3), 28))

list <- model.matrix(~0+pseudobulk_cluster+batch)   ##removing the influence of batch effects on limma differential analysis.
colnames(list) <- c('C1','C2','C3','C4','C5','C6','C7','batch2','batch3')
df.fit <- lmFit(gsva_mat, list)
...
####Heatmap####
gsva_plot=cbind(rowMeans(gsva_mat[,1:12]),rowMeans(gsva_mat[,13:24]),rowMeans(gsva_mat[,25:36]),
                rowMeans(gsva_mat[,37:48]),rowMeans(gsva_mat[,49:60]),rowMeans(gsva_mat[,61:72]),
                rowMeans(gsva_mat[,73:84]))

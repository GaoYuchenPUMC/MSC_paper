####RNA velocity of all 12 MSC samples (Adult and Perinatal) in Fig2####
...
####RNA velocity of all AD-MSC samples in Fig3####
...
barcode=c(ADbarcode1,ADbarcode2,ADbarcode3)

AD=subset(MSC,Tissue=='AD')
emb=AD@reductions$umap@cell.embeddings
row.names(emb)=barcode

cluster.label=Idents(AD)
names(x = cluster.label) <- barcode

ident.colors <- (scales::hue_pal())(n = length(x = levels(x = AD)))
names(x = ident.colors) <- levels(x = AD)
cell.colors <- ident.colors[Idents(object = AD)]
names(x = cell.colors) <- barcode

pca=AD@reductions$pca@cell.embeddings
row.names(pca)=barcode
cell.dist=as.dist(1-armaCor(t(pca)))
...
####RNA velocity of all BM-MSC samples in Fig3####
...
barcode=c(BMbarcode1,BMbarcode2,BMbarcode3)

BM=subset(MSC,Tissue=='BM')
emb=BM@reductions$umap@cell.embeddings
row.names(emb)=barcode

cluster.label=Idents(BM)
names(x = cluster.label) <- barcode
...
pca=BM@reductions$pca@cell.embeddings
row.names(pca)=barcode
cell.dist=as.dist(1-armaCor(t(pca)))
...
####RNA velocity of all PM-MSC samples in Fig3####
...
barcode=c(PMbarcode1,PMbarcode2,PMbarcode3)

PM=subset(MSC,Tissue=='PM')
emb=PM@reductions$umap@cell.embeddings
row.names(emb)=barcode

cluster.label=Idents(PM)
names(x = cluster.label) <- barcode
...
pca=PM@reductions$pca@cell.embeddings
row.names(pca)=barcode
cell.dist=as.dist(1-armaCor(t(pca)))
...
####RNA velocity of all UC-MSC samples in Fig3####
...
barcode=c(UCbarcode1,UCbarcode2,UCbarcode3)

UC=subset(MSC,Tissue=='UC')
emb=UC@reductions$umap@cell.embeddings
row.names(emb)=barcode

cluster.label=Idents(UC)
names(x = cluster.label) <- barcode

ident.colors <- (scales::hue_pal())(n = length(x = levels(x = UC)))
names(x = ident.colors) <- levels(x = UC)
cell.colors <- ident.colors[Idents(object = UC)]
names(x = cell.colors) <- barcode

pca=UC@reductions$pca@cell.embeddings
row.names(pca)=barcode
cell.dist=as.dist(1-armaCor(t(pca)))
